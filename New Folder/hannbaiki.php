...
}else if ($sql_kind === 'update_drink_name') {

  $update_drink_name = '';
  $drink_id     = '';


  if (isset($_POST['update_drink_name']) === TRUE) {
    // 半角・全角空白のトリム
     $update_drink_name = preg_replace('/\A[ \s]*|[ \s]*\z/u', '', $_POST['update_drink_name']);
...
try {
 ...
 if (count($err_msg) === 0 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($sql_kind === 'insert') {
      ...
      try {
        // SQL文を作成
         $sql = 'UPDATE test_drink_master SET img= ? WHERE drink_id = ?';
     ...
     // SQLを実行
      ...
      try {
        // SQL文を作成
         $sql = 'UPDATE test_drink_master SET drink_name= ? WHERE drink_id = ?';
      ...
      // SQL文のプレースホルダに値をバインド
         ...
         $stmt->bindValue(1,$update_drink_name,    PDO::PARAM_STR);
        $stmt->bindValue(2, $drink_id,             PDO::PARAM_INT);
     // SQLを実行
       $stmt->execute();
        var_dump($update_drink_name);
        // 表示メッセージの設定
        $result_msg = '名前変更';
      ...
      try {
        // SQL文を作成
        $sql = 'UPDATE test_drink_master SET price = ? WHERE drink_id = ?';
        // SQL文を実行する準備
        $stmt = $dbh->prepare($sql);
        // SQL文のプレースホルダに値をバインド
...
<?php foreach ($data as $value)  { ?>
      <tr>
        ...
        <td class="drink_name_width">
          <input type="text" class="input_text_width text_align_right" name="update_drink_name" value="<?php print $value['drink_name']; ?>"> &nbsp;&nbsp; <input type="submit"  value= "変更" ></td>
          <input type="hidden" name="sql_kind" value="update_drink_name">
          <input type= "hidden" name="drink_id" value="<?php print $value['drink_id']; ?>">
         </form>
         <form method="post">
