...
if (isset($_POST['sql_kind']) === TRUE) {
  ...
  if (is_uploaded_file($_FILES['new_img']['tmp_name']) === TRUE) {
    // 画像の拡張子を取得
     $extension = pathinfo($_FILES['new_img']['tmp_name'], PATHINFO_EXTENSION);
    // 指定の拡張子であるかどうかチェック
    if ($extension === 'jpg' || $extension === 'png') {
      ...
      if (is_file($img_dir . $new_img) !== TRUE) {
        ...
        if (move_uploaded_file($_FILES['new_img']['name'], $img_dir . $new_img) !== TRUE) {
    ...
    // 1.在庫
        ...
        if (isset($_FILES['update_img']['tmp_name']) === TRUE) {
            if (is_uploaded_file($_FILES['update_img']['name']) === TRUE) {
...
try {
    ...
    $err_msg[] = '予期せぬエラーが発生しました。管理者へお問い合わ���ください。' . $e->getMessage();
}
