...
<meta name="description"
	content="いつもの食卓をちょっぴり豪勢に！主婦からの投稿が多いので、誰でも作れる簡単レシピが沢山掲載されております！">
<meta name="keywords" content="CookCamp,クックキャンプ,レシピ,献立,簡単,料理,recipe">
...
=======
                <th class="now_item"><img
					src="<?php print $img_dir . $value['img']; ?>" height="100" width="100"></br>
				     <a class="now_item">商品名:</a><a><?php print $value['item_name']; ?></a></br> 
                     <a class="now_item">商品価格：<?php print $value['price']; ?>円</a> </br>
					 <a class="now_item">商品在庫：<?php print $value['stock']; ?>個</a>
				</th>
		<th>
             <form method="post" enctype="multipart/form-data">  
                <input type="file" name="update_img" img src="<?php print $img_dir . $value['img']; ?>"
         ...
         <th>
             <form method="post">
				...
				<th>
					<form method="post">
						<input type="text" class="input_text_width"
							name="update_price" value="<?php print $value['price']; ?>">円<input
							type="submit" value="価格変更"> <input type="hidden" name="item_id"
				...
				<th>
					<form method="post">
						<input type="text" class="input_text_width text_align_right"
							name="update_stock" value="<?php print $value['stock']; ?>">個<input
							type="submit" value="変更"> <input type="hidden" name="item_id"
							value="<?php print $value['item_id']; ?>"> <input type="hidden"
							name="sql_kind" value="update_stock">
					</form>
				</th>
				<th>
					<form method="post">
						<input type="submit" value="削除"> <input type="hidden"
							name="sql_kind" value="delete"> <input type="hidden"
...
>>>>>>> 48365b6... コミットメッセージ
                        <?php if ( $value['status'] === '1' ) { ?>
                    <form method="post">
						<input type="submit" value="公開→非公開"> <input type="hidden"
							name="sql_kind" value="change"> <input type="hidden"
							name="change_status" value="0"> <input type="hidden"
							name="item_id" value="<?php print $value['item_id']; ?>">
					</form>
                        <?php } elseif ( $value['status'] === '0' ) { ?>
                    <form method="post">
						<input type="submit" value="非公開→公開"> <input type="hidden"
							name="sql_kind" value="change"> <input type="hidden"
							name="change_status" value="1"> <input type="hidden"
							name="item_id" value="<?php print $value['item_id']; ?>">
					</form>
